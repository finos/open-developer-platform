...
- *Prior to the meeting* - normally 48 hours before - a new GitHub Issue is created using the Meetings issue template; use the `[MEETING]` prefix in the issue title, to help issue browsing and discoverability. The issue URL can be then circulated
...
- *At the end of the meeting* - the meeting leader adds the `meeting` label to the issue, triggering the indexing process; when done, the label `indexed` will be added
...
- If the meeting leader makes a mistake and wants to remove the meeting from the records, the meeting label can simply be removed, and the items will be removed (including the `indexed` label). After fixing assignments, the `meeting` label can be added again.
