...
The Foundation hosts [several Java projects](https://finos.github.io/?languages=Java&sort=hotness-down) and provides a good level of support for building, testing and releasing Java code.
...
You can also adopt other Java build tools, such as [Gradle](https://gradle.org/), [Ant](http://ant.apache.org/), [Leiningen](https://leiningen.org/) or others, assuming that they are able to interact with the Central Repository.
...
## Artifact publishing
All Java projects hosted by the Foundation are expected to use a groupId that is (or prefixed by) `org.finos` and are published on the [Maven central repository](http://search.maven.org/#search%7Cga%7C1%7Corg.finos) (also known as Maven Central).
...
1. Sign up on [https://issues.sonatype.org](https://issues.sonatype.org) (`http://oss.sonatype.org` Maven repo will use these credentials for authentication).
...
```
[![Maven Central](https://img.shields.io/maven-central/v/org.finos/<project name>.svg?maxAge=2592000)](http://search.maven.org/#artifactdetails%7Corg.finos%7C<project name>%7C2%7Cpom)
```
...
3. Promote the staged artifacts by accessing the [Nexus staging repositories](http://central.sonatype.org/pages/releasing-the-deployment.html) to:
...
### Known issues
If the release command fails with "gpg: signing failed: Inappropriate ioctl for device", run the following command and try again; [more info on stackexchange](http://unix.stackexchange.com/a/257065).
```
export GPG_TTY=$(tty)
```
