...
## Artifact publishing
All Java projects hosted by the Foundation are expected to use a groupId that is (or prefixed by) `org.finos.<program name>` and are published on the [Maven central repository](http://search.maven.org/#search%7Cga%7C1%7Corg.finos) (also known as Maven Central).
